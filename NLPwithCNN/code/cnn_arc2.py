...
def init_convnet(layers):
  ...
  c = layers[1]['channel']
  typeOfFilter = 5
  h_list = [h for i in range(typeOfFilter)]
  w_list = [w for i in range(typeOfFilter)]
  c_list = [c for i in range(typeOfFilter)]
  ...
  for i in range(2, len(layers)+1):
    ...
    if layers[i]['type'] == 'CONV':
      for j in range(1, len(layers[i])):
        scale = np.sqrt(3./(h_list[j-1]*w_list[j-1]*c_list[j-1]))
        weight = np.random.rand(
          layers[i][j]['kw'] * layers[i][j]['kh'] * c / layers[i][j]['group'],
          layers[i][j]['num']
        )
        #print "layer: CONV", i, " scale: ", scale, " h w c: ",h_list[j-1], w_list[j-1], c_list[j-1]
        params[i-1][j] = {}
        params[i-1][j]['w'] = 2*scale*weight - scale
        params[i-1][j]['b'] = np.zeros(layers[i][j]['num'])
        # update h, w and c, used in next layer
        #print layers[i][j]['kh'], h_list[j-1], layers[i][j]['pad']
        h_list[j-1] = (h_list[j-1] + 2*layers[i][j]['pad'] - layers[i][j]['kh']) / layers[i][j]['strideh'] + 1
        w_list[j-1] = (w + 2*layers[i][j]['pad'] - layers[i][j]['kw']) / layers[i][j]['stridew'] + 1
        c_list[j-1] = layers[i][j]['num']
    elif layers[i]['type'] == 'POOLING':
      for j in range(1, len(layers[i])):
        #print  "h w: ", h_list[j-1], w_list[j-1]
        h_list[j-1] = (h_list[j-1] - layers[i][j]['kh']) / layers[i][j]['strideh'] + 1
        w_list[j-1] = (w_list[j-1] - layers[i][j]['kw']) / layers[i][j]['stridew'] + 1
        params[i-1][j] = {}
        #print "layer: POOLING", i, " scale: ", scale, " h w: ", h_list[j-1], w_list[j-1]
        params[i-1][j]['w'] = np.array([])
        params[i-1][j]['b'] = np.array([])
      ...
      scale = np.sqrt(3./(h*w*c))
      #print "layer: IP", i, " scale: ", scale, " h w c: ", h, " ",w ," ", c
...
def conv_net(params, layers, data, labels, predict = False):
  """
  ...
  for i in range(2, l):
    if layers[i]['type'] == 'CONV':
      output[i] = {}
      for j in range(1, len(layers[i])):
        output[i][j] = conv_layer_forward(output[i-1], layers[i][j], params[i-1][j])
    elif layers[i]['type'] == 'POOLING':
      output[i] = {}
      for j in range(1, len(layers[i])):
        output[i][j] = pooling_layer_forward(output[i-1][j], layers[i][j])
    elif layers[i]['type'] == 'SUM':
      output[i] = sum_layer_forward(output[i-1], layers[i], params[i-1])
  ...
  if predict:
    return output[l-1], "Predict Result"
  ...
  for i in range(l-1,1,-1):
    ...
    if layers[i]['type'] == 'CONV':
      temp_input_od = input_od
      for j in range(len(output[i])):
        output[i][j+1]['diff'] = temp_input_od[j+1]
        param_grad[i-1][j+1], input_od = conv_layer_backward(output[i][j+1],
                                                        output[i-1],
                                                        layers[i][j+1],
                                                        params[i-1][j+1])
    elif layers[i]['type'] == 'POOLING':
      ...
      for j in range(len(output[i])):
        output[i][j+1]['diff'] = temp_input_od[j+1]
        input_od[j+1] = pooling_layer_backward(output[i][j+1],
                                          output[i-1][j+1],
                                          layers[i][j+1])
        param_grad[i-1][j+1] = {}
        param_grad[i-1][j+1]['w'] = np.array([])
        param_grad[i-1][j+1]['b'] = np.array([])
    elif layers[i]['type'] == 'IP':
      ...
      output[i]['diff'] = input_od
      input_od = sum_layer_backward(output[i], output[i-1], layers[i])
    ...
    if layers[i]['type'] == 'POOLING' or layers[i]['type'] == 'CONV':
      for j in range(1, len(layers[i])):
        param_grad[i-1][j]['w'] = param_grad[i-1][j]['w'] / batch_size
        param_grad[i-1][j]['b'] = param_grad[i-1][j]['b'] / batch_size
      continue
    param_grad[i-1]['w'] = param_grad[i-1]['w'] / batch_size
    param_grad[i-1]['b'] = param_grad[i-1]['b'] / batch_size
...
def im2col_conv(input_n, layer, h_out, w_out):
  ...
  pad = layer['pad']
  #print input_n['data'].shape, h_in, w_in, c
...
#########  YOU SHOULD MAINTAIN THE RETURN TYPE AND SHAPE AS PROVIDED IN STARTER CODE   ######
#############################################################################################
def sum_layer_forward(input, layer, param):
  ...
  output['batch_size'] = input[1]['batch_size']
  data_out = np.zeros((1, output['batch_size'])) 


  for i in range(len(input)):
    data = input[i+1]['data']
    data_out = np.vstack((data_out, data))
  output['data'] = data_out[1:, :]
  return output
...
#########  YOU SHOULD MAINTAIN THE RETURN TYPE AND SHAPE AS PROVIDED IN STARTER CODE   ######
#############################################################################################
def sum_layer_backward(output, input, layer):
# input_od = sum_layer_backward(output[i], output[i-1], layers[i])
  input_od = {}
  for i in range(len(input)):
    input_od[i+1] = np.zeros(input[i+1]['data'].shape)
  diff = output['diff']
  count = 0
  for i in range(len(input_od)):
    for j in range(input[i+1]['data'].shape[0]):
      for k in range(input[i+1]['data'].shape[1]):
        input_od[i+1][j, k] = diff[count, k]
      count += 1
  return input_od
...
def inner_product_forward(input, layer, param):
  ...
  for b in range(batch_size):
    ...
    #print "shape: w, data", w.shape, data_temp.shape
  ...
  for i in range(len(params)):
    if i == 0 or i == 1:
      for j in range(len(params[i+1])):
        theta_w = param_winc[i+1][j+1]['w']
        w = params[i+1][j+1]['w']
        grad_w = param_grad[i+1][j+1]['w'] + decay * w
        new_theta_w = mu * theta_w + w_rate * grad_w
        params_[i+1][j+1]['w'] -= new_theta_w
        param_winc_[i+1][j+1]['w'] = new_theta_w
        # compute new bias
        theta_b = param_winc[i+1][j+1]['b']
        grad_b = param_grad[i+1][j+1]['b']
        new_theta_b = (mu * theta_b + b_rate * grad_b)
        params_[i+1][j+1]['b'] -= new_theta_b
        param_winc_[i+1][j+1]['b'] = new_theta_b
      continue
